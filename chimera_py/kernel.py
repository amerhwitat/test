...
class ChimeraKernel:
    ...
    def __post_init__(self)->None:
        self.core=ChimeraCore(self.cpu_count,max(32,self.cpu_count*4),self.memory_size)
        self.dma=DMAEngine(self.core.memory); self.network=NetworkBridge(); self.nbit=NBitRuntime(8192); self.kore=KoreService()
        self.tasks:Dict[int,Task]={}; self.next_pid=1; self.booted=False; self.boot_time=0.0
    def boot(self)->None:
        self.boot_time=time.monotonic(); self.booted=True; self.kore.publish('kernel.boot',{'cpus':self.cpu_count,'memory':self.memory_size})

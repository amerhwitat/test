...
OPCODES:Dict[str,int]={n:i+1 for i,n in enumerate(_OPCODE_NAMES)}
...
class ExecuteStatus(str,Enum):
    EXECUTED='Executed'; PRIVILEGE_VIOLATION='PrivilegeViolation'; INVALID_OPCODE='InvalidOpcode'; UNIMPLEMENTED_SERVICE='UnimplementedService'

@dataclass(frozen=True)
class Instruction:
    opcode:int; dst:int; src_a:int; src_b:int; imm:int=0
    @property
    def mnemonic(self)->str:return NAMES.get(self.opcode,'')
    def encode(self)->bytes:return self.opcode.to_bytes(2,'little')+self.dst.to_bytes(2,'little')+self.src_a.to_bytes(2,'little')+self.src_b.to_bytes(2,'little')+(self.imm&((1<<64)-1)).to_bytes(8,'little')
    @classmethod
    def decode(cls,data:bytes)->'Instruction':
        if len(data)<16:raise ValueError('canonical Chimera-II instruction requires 16 bytes')
...
@dataclass
class CPU8192:
    gpr_count:int=1024; pc:int=0; flags:int=0; privileged:bool=False
    def __post_init__(self)->None:
        if self.gpr_count<8:raise ValueError('gpr_count must be at least 8')
        self.gpr=[0]*self.gpr_count; self.halted=False; self.instructions=0
    ...
    @staticmethod
    def lane(value:int,index:int)->int:return (value>>(index*64))&((1<<64)-1)
    @staticmethod
    ...
    def rotate_left(value:int,amount:int)->int:
        amount%=8192; return ((value<<amount)|(value>>(8192-amount)))&MASK if amount else value&MASK
    @staticmethod
    def rotate_right(value:int,amount:int)->int:
        amount%=8192; return ((value>>amount)|(value<<(8192-amount)))&MASK if amount else value&MASK
    def execute(self,ins:Instruction,memory:Optional[bytearray]=None)->ExecuteStatus:
        ...
        self.instructions+=1;return ExecuteStatus.EXECUTED
    def _memory_op(self,name:str,ins:Instruction,memory:bytearray)->None:
        addr=ins.imm%len(memory)
        if name=='LOAD':self.write(ins.dst,int.from_bytes(memory[addr:addr+1024].ljust(1024,b'\0'),'little'))
        elif name=='STORE':
            data=self.read(ins.src_a).to_bytes(1024,'little');size=min(1024,max(1,(ins.imm>>48) or 1024));memory[addr:addr+size]=data[:size]
        elif name=='MEMCPY':
            src=self.read(ins.src_a)%len(memory);size=min(len(memory)-addr,ins.imm&0xffff,len(memory)-src);memory[addr:addr+size]=memory[src:src+size]
        elif name=='MEMSET':

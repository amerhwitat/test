...
class BrainGraph:
    ...
    def __post_init__(self) -> None:
        self.activity = self.activity or [0] * self.node_count
        self.rebuild()
...
class ChimeraCore:
    ...
    def __post_init__(self)->None:
        ...
        self.cpus=[CPU8192(self.gpr_count) for _ in range(self.cpu_count)]
        self.memory=bytearray(self.memory_size); self.brain=BrainGraph(self.node_count)
        self.ticks=self.messages_sent=self.messages_delivered=self.scheduler_cursor=0
        self.run_state='paused'; self.lock=threading.RLock()
    def reset(self)->None:
        with self.lock:
            self.cpus=[CPU8192(self.gpr_count) for _ in range(self.cpu_count)]
            self.memory[:]=b'\0'*len(self.memory); self.brain=BrainGraph(self.node_count)
            self.ticks=self.messages_sent=self.messages_delivered=self.scheduler_cursor=0; self.run_state='paused'
    def step(self,code:bytes|None=None,privileged:bool|None=None)->ExecuteStatus:
        with self.lock:
            cpu_id=self.scheduler_cursor%self.cpu_count; cpu=self.cpus[cpu_id]; status=ExecuteStatus.EXECUTED
            if not cpu.halted:
                if code is None:
                    cpu.pc += 4; cpu.instructions += 1
                elif cpu.pc >= len(code):
                    cpu.halted=True
                else:
    ...
    def run(self,code:bytes|None=None,steps:int=1,privileged:bool=False)->List[ExecuteStatus]:
